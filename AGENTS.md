...
- Environment variables (loaded via `internal/config`):
  ...
  - Polling control: `MCP_POLL_INITIAL_SECONDS`, `MCP_POLL_MAX_SECONDS`, `MCP_POLL_TIMEOUT_SECONDS`, `MCP_POLL_BACKOFF_FACTOR` (defaults: 3s, 30s, 30m, 1.5x) govern branch status polling and help avoid premature timeouts on long tasks.
