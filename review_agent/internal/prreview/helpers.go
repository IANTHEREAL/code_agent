...
func buildLogicAnalystPrompt(task string, issueText string, changeAnalysisPath string) string {
	...
	sb.WriteString("- P1 (High): Reachable in realistic production scenarios, and significantly impairs core/major functionality (including user-visible correctness errors), or causes a severe performance regression that impacts use; a workaround may exist but is costly/risky/high-friction. Must be fixed before release.\n")
	sb.WriteString("- Lightweight evidence bar (guidance): Any P0/P1 claim should include (1) a minimal trigger condition (how it happens) and (2) key code evidence (paths + symbols/branches). If either is missing, default to P2 pending evidence.\n\n")
	...
	sb.WriteString("- For each issue, give a clear verdict: P0 / P1 / P2 / Not an issue.\n")
	sb.WriteString("- Include the most critical supporting evidence (file path + key symbols/logic).\n")
	sb.WriteString("- Provide a one-sentence justification for why it does or does not deserve P0/P1 in real scenarios.\n")
	sb.WriteString("- After # VERDICT, include a \"Severity:\" line using P0/P1/P2/Not an issue.\n\n")
