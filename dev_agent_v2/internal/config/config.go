...
// Pantheon branches can take 1-2 hours; treat that as normal.
const minPollTimeoutSeconds = 7200
const defaultPollTimeoutSeconds = 10800
...
func FromEnv() (AgentConfig, error) {
	...
	pollInitial, err := envSeconds("MCP_POLL_INITIAL_SECONDS", 5)
	if err != nil {
		return AgentConfig{}, err
	}
	pollMax, err := envSeconds("MCP_POLL_MAX_SECONDS", 600)
	if err != nil {
		return AgentConfig{}, err
	}
	pollTimeout, err := envSeconds("MCP_POLL_TIMEOUT_SECONDS", defaultPollTimeoutSeconds)
	if err != nil {
		return AgentConfig{}, err
	}
