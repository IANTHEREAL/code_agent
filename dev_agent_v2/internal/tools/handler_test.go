...
func TestExecuteAgentTruncatesResponseAndProvidesHint(t *testing.T) {
	long := strings.Repeat("x", defaultExecuteResponseMaxChars+250) + "THE_END"
	client := &fakeMCPClient{
		branchOutputResult: map[string]any{"output": long},
	}
	handler := &ToolHandler{
		client:        client,
		defaultProj:   "proj",
	...
	response, _ := res["response"].(string)
	if response == "" {
		t.Fatalf("expected response to be present")
	}
	if len(response) > defaultExecuteResponseMaxChars {
		t.Fatalf("response length %d > max %d", len(response), defaultExecuteResponseMaxChars)
	}
	if !strings.Contains(response, "THE_END") {
		t.Fatalf("expected tail excerpt to contain THE_END, got: %q", response)
	}
	if truncated, _ := res["response_truncated"].(bool); !truncated {
		t.Fatalf("expected response_truncated=true, got %#v", res["response_truncated"])
	}
	hint, ok := res["full_output_hint"].(map[string]any)
	if !ok {
		t.Fatalf("expected full_output_hint map, got %#v", res["full_output_hint"])
	}
	if hint["tool"] != "branch_output" {
		t.Fatalf("unexpected hint tool %#v", hint["tool"])
	}
	hArgs, ok := hint["arguments"].(map[string]any)
	if !ok {
		t.Fatalf("expected hint.arguments map, got %#v", hint["arguments"])
	}
	if hArgs["branch_id"] != "branch-1" {
		t.Fatalf("expected hint branch_id=branch-1, got %#v", hArgs["branch_id"])
	}
	if hArgs["full_output"] != true {
		t.Fatalf("expected hint full_output=true, got %#v", hArgs["full_output"])
	}
	if hArgs["tail"] != true {
		t.Fatalf("expected hint tail=true, got %#v", hArgs["tail"])
	}
	if max, ok := hArgs["max_chars"].(int); !ok || max != defaultBranchOutputMaxChars {
		t.Fatalf("expected hint max_chars=%d, got %#v", defaultBranchOutputMaxChars, hArgs["max_chars"])
	}
	if len(client.branchOutputInputs) == 0 || !client.branchOutputInputs[0].fullOutput {
		t.Fatalf("expected execute_agent to fetch full branch_output for slicing; calls=%#v", client.branchOutputInputs)
	}
}
...
func TestHandleBranchOutputSupportsTailAndMaxChars(t *testing.T) {
	client := &fakeMCPClient{
		branchOutputResult: map[string]any{"output": "abcdefghijklmnopqrstuvwxyz"},
	}
	handler := &ToolHandler{
		client:        client,
		branchTracker: NewBranchTracker("parent"),
	...
	call := ToolCall{}
	call.Function.Name = "branch_output"
	call.Function.Arguments = `{"branch_id":"branch-123","tail":true,"max_chars":5}`
	...
	data, _ := res["data"].(map[string]any)
	if data["output"] != "vwxyz" {
		t.Fatalf("unexpected tail output %#v", data["output"])
	}
	if truncated, _ := data["output_truncated"].(bool); !truncated {
		t.Fatalf("expected output_truncated=true, got %#v", data["output_truncated"])
	}
	if len(client.branchOutputInputs) != 1 || !client.branchOutputInputs[0].fullOutput {
		t.Fatalf("expected tail=true to force full_output fetch; calls=%#v", client.branchOutputInputs)
	}
}
