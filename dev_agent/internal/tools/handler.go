...
func NewToolHandler(client agentClient, defaultProject string, startBranch string, workspaceDir string, timeout time.Duration) *ToolHandler {
	if timeout <= 0 {
...
func (h *ToolHandler) checkStatus(arguments map[string]any) (map[string]any, error) {
	...
	timeout := h.branchTimeout.Seconds()
	if timeout <= 0 {
		timeout = 1800.0
	}
